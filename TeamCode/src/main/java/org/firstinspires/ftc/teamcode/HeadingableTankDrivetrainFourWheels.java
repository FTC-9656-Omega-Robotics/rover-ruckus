...
/*
 * A Four Wheel Tank Drivetrain with some sort of heading sensor, such as a gyro. Uses the sensor and a controller specified by the user to control the system.
 *
 */
public class HeadingableTankDrivetrainFourWheels extends TankDrivetrainFourWheels implements Headingable {
    /**
     ...
     *                   @see edu.spa.ftclib.internal.controller.PIDController
     */
    public HeadingableTankDrivetrainFourWheels(DcMotor[] motors, FinishableIntegratedController controller) {
        super(motors);
        this.controller = controller;
    }

    public HeadingableTankDrivetrainFourWheels(DcMotor motor1, DcMotor motor2, DcMotor motor3, DcMotor motor4, FinishableIntegratedController controller) {
    ...
    /**
     * Set the target heading of the drivetrain.
     * @param degrees number of degrees the robot should turn
     *
     */
    public void setTargetHeadingInDegrees(double degrees) {
        setTargetHeading(degrees * Math.PI/180);
    }

    /**
     * Get the current heading of the drivetrain (presumably a value from a sensor and not necessarily the drivetrain's target heading).
     *
     * @return The angle (deg) the drivetrain is currently facing
     */
    public double getCurrentHeadingInDegrees() {
        return getCurrentHeading() * 180/Math.PI;
    }

    /**
     * Get the target heading of the drivetrain (not necessarily the actual, current heading of the drivetrain), passed in using {@link #setTargetHeading}.
     *
     * @return The heading (deg) that the robot is currently trying to get to or maintain
     */
    public double getTargetHeadingInDegrees() {
        return getTargetHeading() * 180/Math.PI;
    }
